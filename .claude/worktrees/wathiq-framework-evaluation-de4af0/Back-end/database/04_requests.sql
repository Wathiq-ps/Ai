-- =============================================================================
-- Wathiq — 04. Purchase and rental requests
--
-- Module 5 (FR-5.1 .. FR-5.6), UC-020, UC-021, UC-006, UC-007, UC-043, UC-063.
--
-- FR-5.7 does not exist. UC-063 (Close Request) has no functional requirement
-- behind it: nothing in the SRS defines the conditions under which a request
-- closes or what read-only means afterwards. The schema below assumes closure
-- is a terminal state reachable from `accepted` once its contract completes,
-- and from `pending` on property withdrawal. Confirm before M5 ships.
-- =============================================================================
...
-- BR: one live request per (property, requester). Re-applying while one is
-- pending is a duplicate, not a second offer.
create unique index requests_one_pending_per_requester
    on app.property_requests (property_id, requester_id)
    where status = 'pending';

-- BR: a property can have at most one accepted request at a time. Without this
-- two buyers can both be accepted and two contracts open on one property —
-- a race that a check-then-insert in application code will lose under load.
...
-- The requester must not be the owner. Enforced by trigger because the owner
